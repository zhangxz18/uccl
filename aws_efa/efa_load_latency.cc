...
#else
    if (cudaMalloc(&rdma->buf1, (BUFFER_SIZE1 + UD_ADDITION) *
                                    MAX_NUM_BUFFERS) != cudaSuccess) {
...
void run_client(struct rdma_context *rdma, const char *server_ip, int gid_index,
                double target_rate) {
    uint32_t remote_qpn;
    union ibv_gid local_gid, remote_gid;
    ...
    while (sent_count < ITERATION || inflight > 0) {
        ...
        if (inflight < MAX_INFLIGHT && sent_count < ITERATION &&
            ...
            *(uint64_t *)(rdma->buf2 + BUFFER_SIZE2 * buf_idx + 32) =
                sent_count;
            rdma_send(rdma, remote_qpn, buf_idx);
