...
static constexpr uint32_t EFA_HDR_OVERHEAD = (EFA_MTU - EFA_MAX_PAYLOAD);
static constexpr uint32_t BASE_PSN = 0;

// For quick computation at EFA_MTU
static constexpr uint32_t MAX_CHUNK_EFA_HDR_OVERHEAD =
    ((kChunkSize + EFA_MTU) / EFA_MTU) * EFA_HDR_OVERHEAD;
...
class UDPktHdrBuffPool : public BuffPool {
   public:
    ...
    static constexpr uint32_t kNumPktHdr =
        kCQSize * 2;  // Send and Recv buffers.
...
class UDPktDataBuffPool : public BuffPool {
   public:
    ...
    static constexpr uint32_t kNumPktData =
        kCQSize * 2;  // Send and Recv buffers.
...
class TXTracking {
   public:
    ...
    uint64_t ack_transmitted_chunks(uint32_t engine_offset, uint32_t g_qpidx,
                                    uint32_t num_acked_chunks, uint64_t t5,
                                    uint64_t t6, uint64_t remote_queueing_tsc);

    inline void track_chunk(struct ucclRequest *ureq, uint32_t csn,
                            struct wr_ex *wr_ex, uint64_t timestamp) {
...
/**
 * @brief UDQPWrapper is a wrapper for ibv_qp with additional information for
...
struct UDQPWrapper {
    ...
    // We use list_empty(&qpw->ack.ack_link) to check if it has pending ACK to
    // send.
    struct ack_item ack;
 ...
 *   - (UD QP): Multiple Unreliable Datagram QPs and a shared CQ. Different DC
 * QPs have different RQ.
 *   - (Ctrl QP): A high-priority QP for control messages and a dedicated CQ,
 * PD, and MR.
 */
class RDMAContext {
   public:
    constexpr static int kTotalQP = kPortEntropy + 1;
    ...
    // Try to arm a timer for the given QP. If the timer is already armed, do
    // nothing.
    inline void arm_timer_for_qp(struct UDQPWrapper *qpw) {
        if (!qpw->rto_armed) {
            if constexpr (kConstRTO) {
                ...
                rto_->arm_timer(
                    {this, qpw},
                    std::max(kRTORTT * qpw->pcb.timely.get_avg_rtt(),
                             kMinRTOUsec));
            }
            qpw->rto_armed = true;
        }
    ...
    inline void rearm_timer_for_qp(struct UDQPWrapper *qpw) {
        if (qpw->rto_armed) {
            if constexpr (kConstRTO) {
    ...
    inline void mark_qp_timeout(struct UDQPWrapper *qpw) {
        qpw->rto_armed = false;
    }

    inline void disarm_timer_for_qp(struct UDQPWrapper *qpw) {
        if (qpw->rto_armed) {
    ...
    // QPs for data transfer based on Unreliable Datagram (UD).
    struct UDQPWrapper ud_qps_[kPortEntropy];
    // UC QPN to index mapping.
    std::unordered_map<uint32_t, int> qpn2idx_;

    // Shared CQ for all UD QPs.
    ...
    // Global timing wheel for all UD QPs.
    TimingWheel wheel_;
    ...
    // Memory regions for UD packets.
    struct ibv_mr *pkt_hdr_mr_;
    struct ibv_mr *pkt_data_mr_;

    // Buffer pool for UD packet.
    std::optional<UDPktHdrBuffPool> pkt_hdr_pool_;
    std::optional<UDPktDataBuffPool> pkt_data_pool_;
    ...
    // Pre-allocated WQEs for receiving data.
    struct ibv_recv_wr rq_wrs_[kPostRQThreshold];
    ...
    inline uint64_t convert_nic_to_host(uint64_t host_clock,
                                        uint64_t nic_clock) {
    ...
    inline uint32_t select_qpidx_pow2(void) {
        ...
        return ud_qps_[q1].pcb.timely.prev_rtt_ <
                       ud_qps_[q2].pcb.timely.prev_rtt_
                   ? q1
                   : q2;
    }
     ...
     * @param wr_ex
     */
    void retransmit_chunk(struct UDQPWrapper *qpw, struct wr_ex *wr_ex);
     ...
     * @param qpw
     */
    void try_update_csn(struct UDQPWrapper *qpw);
     ...
     * @param rto
     */
    void __retransmit(struct UDQPWrapper *qpw, bool rto);
    inline void fast_retransmit(struct UDQPWrapper *qpw) {
        __retransmit(qpw, false);
    }
    inline void rto_retransmit(struct UDQPWrapper *qpw) {
        __retransmit(qpw, true);
    }

    std::string to_string();

    RDMAContext(TimerManager *rto, int dev, uint32_t engine_offset,
                union CtrlMeta meta);
...
class RDMAFactory {
   ...
   public:
     ...
     * @return RDMAContext*
     */
    static RDMAContext *CreateContext(TimerManager *rto, int dev,
                                      uint32_t engine_offset_,
                                      union CtrlMeta meta);
 ...
 * @return int
 */
static inline int util_rdma_get_ib_name_from_gididx(int gid_idx,
                                                    char *ib_name) {
