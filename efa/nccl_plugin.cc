...
// the call succeeds.
ncclResult_t pluginListen(int dev, void *opaque_handle, void **listenComm) {
    ...
    *listenComm = lcomm;
    ...
    LOG(INFO) << "pluginListen on dev: " << dev;
...
ncclResult_t pluginConnect(int dev, void *opaque_handle, void **sendComm,
    ...
    if (handle->state == kConnInit) {
        LOG(INFO) << "pluginConnect on dev: " << dev << " remote_ip_str "
                  << remote_ip_str;
    ...
    if (*sendComm) {
        printf("Connected to %s/%d on dev:%d, %lx\n", remote_ip_str.c_str(),
...
ncclResult_t pluginAccept(void *listenComm, void **recvComm,
    ...
    if (lcomm->state == kConnInit) {
        LOG(INFO) << "pluginAccept on dev: " << lcomm->dev;
    ...
    if (*recvComm) {
        printf("Accepted from %s/%d on dev:%d, %lx\n",
...
ncclResult_t pluginRegMr(void *collComm, void *data, size_t size, int type,
    ...
    LOG(INFO) << "pluginRegMr, " << size << ", 0x" << std::hex
              << base->conn_id.flow_id;
    reg_cnt++;
...
ncclResult_t pluginRegMrDmaBuf(void *collComm, void *data, size_t size,
    ...
    LOG(INFO) << "pluginRegMrDmaBuf, " << size << ", 0x" << std::hex
              << base->conn_id.flow_id;
    reg_cnt++;
...
ncclResult_t pluginDeregMr(void *collComm, void *mhandle) {
    ...
    LOG(INFO) << "pluginDeregMr, 0x" << std::hex << base->conn_id.flow_id;
...
ncclResult_t pluginIsend(void *sendComm, void *data, int size, int tag,
    ...
    // checkMemoryLocation(data);
    // LOG(INFO) << "Isend data: " << *(uint32_t *)data;
    ...
    *request = req;
    ...
    VLOG(3) << "pluginIsend on dev: " << dev << ", size: " << size
            << ", flow 0x" << std::hex << conn_id.flow_id;
...
ncclResult_t pluginIrecv(void *recvComm, int n, void **data, int *sizes,
    ...
    // checkMemoryLocation(data);
    // LOG(INFO) << "Irecv data: " << *(uint32_t *)data[0];
    ...
    *request = req;
    ...
    VLOG(3) << "pluginIrecv on dev: " << dev << ", size: " << sizes[0]
            << ", flow 0x" << std::hex << conn_id.flow_id;
