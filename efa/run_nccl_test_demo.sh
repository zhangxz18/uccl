...
if [ "$BIN" = "all_reduce_perf" ]; then
    if [ "$TEST" = "srd" ]; then
        ...
        LIBNCCL_PATH="/opt/uccl_rdma_ar/nccl/build/lib/libnccl.so"
        PLUGIN_PATH="/opt/uccl_rdma_ar/efa/libnccl-net.so"
        # COPY_CHANNELS=8
        COPY_CHANNELS=4
        CHANNELS_NET_PEER=1
        CHUNK_SIZE=524288
    ...
    if [ "$TEST" = "srd" ]; then
        ...
        LIBNCCL_PATH="/opt/uccl_rdma_a2a/nccl/build/lib/libnccl.so"
        PLUGIN_PATH="/opt/uccl_rdma_a2a/efa/libnccl-net.so"
