...
static void server_setup_func(int local_rank, int target_rank, bool incast)
{
    ...
    auto fd = incast ? nodes[target_rank].incast_listen_fd_ : nodes[target_rank].listen_fd_;
...
static void client_setup_func(int local_rank, int target_rank, bool incast)
{
    ...
    int base_listen_port = (incast ? INCAST_BASE_LISTEN_PORT : BASE_LISTEN_PORT) + target_rank * 32;

    auto local_dev = local_rank % NB_THREADS;
    auto target_dev = target_rank % NB_THREADS;

    if (incast)
        MPI_LOG(INFO) << "Incast, " << local_rank << " --> " << target_rank << ", port: " << base_listen_port + local_rank;
    else {
        MPI_LOG(INFO) << "PT, " << local_rank << " --> " << target_rank << ", port: " << base_listen_port + local_rank;
    }

    auto conn_id = ep->uccl_connect(local_dev, local_rank, target_dev, target_rank, ip, base_listen_port + local_rank);
...
{    
    ...
    #ifdef INCAST
    {
        if (local_rank == INCAST_RANK) {
            // This is incast rank, accept connections from all nodes except for itself.
...
void pt_register_recv_buffer(int local_rank, int r)
{
    ...
    #else
    nodes[r].recv_comm_.pt.buffer = mmap(nullptr, MAX_BUFFER_SIZE, PROT_READ | PROT_WRITE, MAP_PRIVATE | MAP_ANONYMOUS, -1, 0);
    DCHECK(nodes[r].recv_comm_.pt.buffer != MAP_FAILED);
    #endif

    ep->uccl_regmr((UcclFlow *)nodes[r].recv_comm_.pt.conn_id.context, nodes[r].recv_comm_.pt.buffer,
    MAX_BUFFER_SIZE, 0, &nodes[r].recv_comm_.pt.mhandle);
}
...
void pt_register_send_buffer(int local_rank, int r)
{
    ...
    #else
    nodes[r].send_comm_.pt.buffer = mmap(nullptr, MAX_BUFFER_SIZE, PROT_READ | PROT_WRITE, MAP_PRIVATE | MAP_ANONYMOUS, -1, 0);
    DCHECK(nodes[r].send_comm_.pt.buffer != MAP_FAILED);
    #endif

    ep->uccl_regmr((UcclFlow *)nodes[r].send_comm_.pt.conn_id.context, nodes[r].send_comm_.pt.buffer,
    MAX_BUFFER_SIZE, 0, &nodes[r].send_comm_.pt.mhandle);
}
...
void incast_register_recv_buffer(int local_rank, int r)
{
    ...
    #else
    nodes[r].recv_comm_.incast[local_rank].buffer = mmap(nullptr, MAX_BUFFER_SIZE, PROT_READ | PROT_WRITE, MAP_PRIVATE | MAP_ANONYMOUS, -1, 0);
    DCHECK(nodes[r].recv_comm_.incast[local_rank].buffer != MAP_FAILED);
    #endif

    ep->uccl_regmr((UcclFlow *)nodes[r].recv_comm_.incast[local_rank].conn_id.context, nodes[r].recv_comm_.incast[local_rank].buffer,
    MAX_BUFFER_SIZE, 0, &nodes[r].recv_comm_.incast[local_rank].mhandle);
}
...
void incast_register_send_buffer(int local_rank, int r)
{
    ...
    #else
    nodes[r].send_comm_.incast[local_rank].buffer = mmap(nullptr, MAX_BUFFER_SIZE, PROT_READ | PROT_WRITE, MAP_PRIVATE | MAP_ANONYMOUS, -1, 0);
    DCHECK(nodes[r].send_comm_.incast[local_rank].buffer != MAP_FAILED);
    #endif

    ep->uccl_regmr((UcclFlow *)nodes[r].send_comm_.incast[local_rank].conn_id.context, nodes[r].send_comm_.incast[local_rank].buffer,
    MAX_BUFFER_SIZE, 0, &nodes[r].send_comm_.incast[local_rank].mhandle);
}
...
void pt_listen_ports(int local_rank)
{
    ...
    MPI_LOG(INFO) << local_rank << " listen on port " << base_listen_port + r << " for PT.";
}
...
void incast_listen_ports()
{
    ...
    for (int r = 0; r < NRANKS; r++) {
        ...
        int flag = 1;
        DCHECK(setsockopt(nodes[r].incast_listen_fd_, SOL_SOCKET, SO_REUSEADDR, &flag,
                          sizeof(int)) >= 0);
        struct sockaddr_in serv_addr;
        bzero((char *)&serv_addr, sizeof(serv_addr));
        serv_addr.sin_family = AF_INET;
        serv_addr.sin_addr.s_addr = INADDR_ANY;
        serv_addr.sin_port = htons(base_listen_port + r);
        int ret = bind(nodes[r].incast_listen_fd_, (struct sockaddr *)&serv_addr, sizeof(serv_addr));
        DCHECK(ret >= 0) << ret;
        ret = listen(nodes[r].incast_listen_fd_, MAX_NODES);
        DCHECK(ret == 0) << ret;
        
        MPI_LOG(INFO) << INCAST_RANK << " listen on port " << base_listen_port + r << " for incast.";
    }
}
...
bool test_p2p_send_done(int local_rank, int target_rank, int chunk_id, bool incast)
{
    bool done;
    ...
    if (incast) {
        ...
        done = ep->uccl_poll_ureq_once(&send_comm_->incast[local_rank].ureq[chunk_id]);
        if (done) {
...
bool test_p2p_recv_done(int local_rank, int target_rank, int chunk_id, bool incast)
{
    bool done;
    ...
    if (incast) {
        ...
        done = ep->uccl_poll_ureq_once(&recv_comm_->incast[local_rank].ureq[chunk_id]);
        if (done) {
            incast_rx_cur_sec_bytes[local_rank].fetch_add(recv_comm_->incast[local_rank].ureq[chunk_id].recv.data_len[0]);
        ...
        if (done) {
            pt_rx_cur_sec_bytes[local_rank].fetch_add(recv_comm_->pt.ureq[chunk_id].recv.data_len[0]);
...
void test_all_send(int local_rank, int target_rank, bool incast)
{
    ...
    if (incast) {
        nb_net_chunk = nodes[target_rank].send_comm_.incast[local_rank].nb_net_chunk;
    } else {
...
void test_all_recv(int local_rank, int target_rank, bool incast)
{
    uint32_t nb_net_chunk;
    if (incast) {
        nb_net_chunk = nodes[target_rank].recv_comm_.incast[local_rank].nb_net_chunk;
    } else {
...
bool nb_test_all_send(int local_rank, int target_rank, bool incast)
{
    ...
    uint32_t ready = 0;
    for (int i = 0; i < nb_net_chunk; i++) {
        if (test_p2p_send_done(local_rank, target_rank, i, incast))
            ready++;
    }
...
bool nb_test_all_recv(int local_rank, int target_rank, bool incast)
{
    ...
    uint32_t ready = 0;
    for (int i = 0; i < nb_net_chunk; i++) {
        if (test_p2p_recv_done(local_rank, target_rank, i, incast))
            ready++;
    }
    return ready == nb_net_chunk;
}
    ...
    stats_thread = std::thread ([&] {
        while (!quit) {
            ...
            for (int i = 0; i < NB_THREADS; i++) {
                ...
                printf(
                    "Rank %d (Permutation): TX Tput: %.4f Gbps, RX Tput: %.4f Gbps, iterations: %d\n",
                    local_rank,
                    (pt_tx_cur_sec_bytes[local_rank].load() - pt_tx_prev_sec_bytes[local_rank]) * 8.0 / 1e9,
                    (pt_rx_cur_sec_bytes[local_rank].load() - pt_rx_prev_sec_bytes[local_rank]) * 8.0 / 1e9,
                    pt_cur_iteration[local_rank]);
    
                    pt_tx_prev_sec_bytes[local_rank] = pt_tx_cur_sec_bytes[local_rank].load();
                    pt_rx_prev_sec_bytes[local_rank] = pt_rx_cur_sec_bytes[local_rank].load();
            }
            #endif
            ...
            for (int i = 0; i < NB_THREADS; i++) {
                int local_rank = i + NODE_ID * NB_THREADS;
                printf(
                    "Rank %d (Incast): TX Tput: %.4f Gbps, RX Tput: %.4f Gbps, iterations: %d\n",
                    local_rank,
                    (incast_tx_cur_sec_bytes[local_rank].load() - incast_tx_prev_sec_bytes[local_rank]) * 8.0 / 1e9,
                    (incast_rx_cur_sec_bytes[local_rank].load() - incast_rx_prev_sec_bytes[local_rank]) * 8.0 / 1e9,
                    incast_cur_iteration[local_rank]);
    
                    incast_tx_prev_sec_bytes[local_rank] = incast_tx_cur_sec_bytes[local_rank].load();
                    incast_rx_prev_sec_bytes[local_rank] = incast_rx_cur_sec_bytes[local_rank].load();
            }
            #endif
        }
        return 0;
...
void permutation_traffic()
{
    ...
    for (int i = 0; i < NB_THREADS; i++) {
        int local_rank = i + NODE_ID * NB_THREADS;
        ts.push_back(std::move(std::thread(permutation_traffic_rank_thread, local_rank)));
    }
