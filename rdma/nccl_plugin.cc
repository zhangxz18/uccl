...
static uint32_t accept_id[NUM_DEVICES] = {};
static inline uint32_t alloc_aid(int dev) {return ++accept_id[dev];}
std::mutex accept_mtx[NUM_DEVICES] = {};

static uint32_t connect_id[NUM_DEVICES] = {};
static inline uint32_t alloc_cid(int dev) {return ++connect_id[dev];}
std::mutex connect_mtx[NUM_DEVICES] = {};
...
std::unordered_map<uint32_t, struct AsyncAcceptState *> accept_tracker[NUM_DEVICES];
...
std::unordered_map<uint32_t, struct AsyncConnectState *> connect_tracker[NUM_DEVICES]; 
                           ...
                           ncclNetDeviceHandle_v8_t** /*sendDevComm*/)
{
    ...
    if (handle->cid == 0) {
        ...
        connect_state->th = std::thread([dev, connect_state, remote_ip_str, handle] {
            connect_state->base.conn_id = ep->uccl_connect(dev, remote_ip_str, handle->remote_dev, handle->listen_port);
            connect_state->base.dev = dev;
            std::atomic_thread_fence(std::memory_order_release);
            std::atomic_store_explicit(&connect_state->done, true, std::memory_order_relaxed);
        });
        connect_tracker[dev][handle->cid] = connect_state;
        *sendComm = nullptr;
        free(scomm);
    } else {
        auto it = connect_tracker[dev].find(handle->cid);
        DCHECK (it != connect_tracker[dev].end());
        // Check if the connection setup is done.
        auto connect_state = it->second;
        auto done = std::atomic_load_explicit(&connect_state->done, std::memory_order_relaxed);
        if (done) {
            std::atomic_thread_fence(std::memory_order_release);
            scomm->base = connect_state->base;
            *sendComm = scomm;
            connect_state->th.join();
            delete connect_state;
            connect_tracker[dev].erase(it);
        } else {
            *sendComm = nullptr;
            free(scomm);
        }
...
ncclResult_t pluginAccept(void* listenComm, void** recvComm,
    ...
    if (it == accept_tracker[lcomm->dev].end()) {
        // Delegate connection to another thread.
        auto accept_state = new struct AsyncAcceptState();
        accept_state->th = std::thread([lcomm, accept_state] {
            std::string remote_ip_str;
            int remote_dev;
            accept_state->base.conn_id = ep->uccl_accept(lcomm->dev, lcomm->listen_fd, remote_ip_str, &remote_dev);
            accept_state->base.dev = lcomm->dev;
            accept_state->remote_ip_str = remote_ip_str;
            accept_state->remote_dev = remote_dev;
            std::atomic_thread_fence(std::memory_order_release);
            std::atomic_store_explicit(&accept_state->done, true, std::memory_order_relaxed);
        });
        accept_tracker[lcomm->dev][lcomm->aid] = accept_state;
        *recvComm = nullptr;
        free(rcomm);
    } else {
        // Check if the connection setup is done.
        auto accept_state = it->second;
        auto done = std::atomic_load_explicit(&accept_state->done, std::memory_order_relaxed);
        if (done) {
            std::atomic_thread_fence(std::memory_order_release);
            rcomm->base = accept_state->base;
            rcomm->remote_ip_str = accept_state->remote_ip_str;
            rcomm->remote_dev = accept_state->remote_dev;
            *recvComm = rcomm;
            accept_state->th.join();
            delete accept_state;
            accept_tracker[lcomm->dev].erase(it);
        } else {
            *recvComm = nullptr;
            free(rcomm);
        }
