...
static uint32_t global_conn_idx[NUM_DEVICES] = {};
...
ncclResult_t pluginListen(int dev, void* opaqueHandle, void** listenComm)
{
    ...
    handle->conn_idx = global_conn_idx[dev]++;
                           ...
                           ncclNetDeviceHandle_v8_t** /*sendDevComm*/)
{
    ...
    auto it = connect_tracker[dev].find(handle->conn_idx);
    if (it == connect_tracker[dev].end()) {
        ...
        connect_tracker[dev][handle->conn_idx] = connect_state;
        *sendComm = nullptr;
    } else {
...
ncclResult_t pluginAccept(void* listenComm, void** recvComm,
    ...
    auto it = accept_tracker[lcomm->dev].find(lcomm->conn_idx);
    if (it == accept_tracker[lcomm->dev].end()) {
        ...
        accept_tracker[lcomm->dev][lcomm->conn_idx] = accept_state;
        *recvComm = nullptr;
    } else {
        // Check if the connection setup is done.
