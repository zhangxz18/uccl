...
ncclResult_t pluginListen(int dev, void* opaqueHandle, void** listenComm)
{
    ...
    {
        std::lock_guard<std::mutex> lock(conn_idx_mtx);
        handle->conn_idx = global_conn_idx++;
    }
...
std::mutex accept_tracker_mtx;
std::unordered_map<uint32_t, struct AsyncAcceptState *> accept_tracker;
...
std::mutex connect_tracker_mtx;
std::unordered_map<uint32_t, struct AsyncConnectState *> connect_tracker; 
                           ...
                           ncclNetDeviceHandle_v8_t** /*sendDevComm*/)
{
    ...
    std::lock_guard<std::mutex> lock(connect_tracker_mtx);
    auto it = connect_tracker.find(handle->conn_idx);
    if (it == connect_tracker.end()) {
        ...
        connect_tracker[handle->conn_idx] = connect_state;
        *sendComm = nullptr;
    } else {
...
ncclResult_t pluginAccept(void* listenComm, void** recvComm,
    ...
    std::lock_guard<std::mutex> lock(accept_tracker_mtx);
    auto it = accept_tracker.find(lcomm->conn_idx);
    if (it == accept_tracker.end()) {
        // Delegate connection to another thread.
        ...
        accept_tracker[lcomm->conn_idx] = accept_state;
        *recvComm = nullptr;
    } else {
        // Check if the connection setup is done.
