...
struct swift_record_t {
 ...
 class SwiftCC {
   public:
    ...
    static constexpr uint32_t kDefaultWnd = 32 * kMSS;
    ...
    static constexpr double kBaseDelay = 20;             // in microseconds

    // flow scaling
    static constexpr double kFSRange = 5 * kBaseDelay;
    static constexpr double kFSMinCwnd = 0.1;           // in MTU-sized packets
    ...
    uint32_t swift_cwnd_ = kDefaultWnd;
     ...
     SwiftCC(double freq_ghz, double link_bandwidth)
         : last_decrease_tsc_(rdtsc()), min_rtt_tsc_(kBaseDelay * freq_ghz * 1000),
            freq_ghz_(freq_ghz), link_bandwidth_(link_bandwidth), create_tsc_(rdtsc()) {
     ...
     void adjust_wnd(double delay, uint32_t acked_bytes) {
        ...
        // LOG_EVERY_N(ERROR, 100) << "Average RTT: " << rtt_ << " us, min:" << min_rtt << " us";
        ...
        if (delay < target_delay) {
            ...
            swift_cwnd_ = swift_cwnd_ * std::max(1 - kBeta * (delay - target_delay) / delay, 1 - kMaxDF);
        }

        if (swift_cwnd_ < kMinCwnd)
            swift_cwnd_ = kMinCwnd;
        if (swift_cwnd_ > kMaxCwnd)
            swift_cwnd_ = kMaxCwnd;
     }
