...
void runServer() {
    ...
    while (true) {
        ...
        LOG(INFO) << "Connection accepted from "
                  << inet_ntoa(clientAddr.sin_addr) << ":"
                  << htons(clientAddr.sin_port);

        for (int j = 0; j < NUM_MESSAGES; ++j) {
            receive_message(BUFFER_SIZE, clientSocket, (uint8_t *)buffer,
                            &quit);
            send_message(sizeof(confirmation), clientSocket,
                         (uint8_t *)confirmation, &quit);
        }
...
void runClient(const std::string &serverIP) {
    ...
    for (int i = 0; i < NUM_ITERATIONS; ++i) {
        ...
        for (int j = 0; j < NUM_MESSAGES; ++j) {
            ...
            int ret = send_message(BUFFER_SIZE, clientSocket,
                                   (uint8_t *)message, &quit);
            ret = receive_message(sizeof(response), clientSocket,
                                  (uint8_t *)response, &quit);
    ...
    LOG(INFO) << "Tail of tail Latencies: "
