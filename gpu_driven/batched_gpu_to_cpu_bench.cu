...
static constexpr int kNumThBlocks = 2;
...
struct alignas(128) Fifo {
    volatile uint32_t head; // Next slot to produce
    volatile uint32_t tail; // Next slot to consume
...
__device__ __forceinline__ uint32_t atomicCAS_sys(volatile uint32_t *addr, 
                                                   uint32_t cmp, 
                                                   uint32_t val) {
    return atomicCAS_system(reinterpret_cast<unsigned int*>(const_cast<uint32_t*>(addr)),
                            cmp, val);
}
    ...
    for (int it = 0; it < kIterations; ++it)
    {
        ...
        while (true) {
            ...
            if (cur_head - cur_tail < kQueueSize) {
                if (atomicCAS_sys(&my_fifo->head, cur_head, cur_head + 1) == cur_head) {
                    my_hdr = cur_head; 
...
void cpu_consume(Fifo *fifo, int block_idx) {
    ...
    for (int seen = 0; seen < kIterations; ++seen) {
        ...
        uint32_t idx = my_tail & kQueueMask;
        uint64_t cmd;
        do { 
            cmd = fifo->buf[idx]; 
        } while (cmd == 0);
